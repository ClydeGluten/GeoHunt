...
export const caughtBehaviorEnum = pgEnum("caught_behavior", [
  "SEEKER",
  "SPECTATOR",
]);
export const boundaryAudienceEnum = pgEnum("boundary_audience", [
  "HOST",
  "SEEKERS",
  "ALL",
]);
...
export const accounts = pgTable(
  ...
  (table) => [
    uniqueIndex("accounts_telegram_user_id_uq").on(table.telegramUserId),
  ],
);
...
export const matches = pgTable(
  "matches",
  {
    ...
    pausedDurationMs: bigint("paused_duration_ms", { mode: "number" })
      .default(0)
      .notNull(),
  ...
  (table) => [
    index("matches_host_idx").on(table.hostAccountId),
...
export const telegramChatOwnerships = pgTable(
  ...
  (table) => [
    primaryKey({ columns: [table.telegramChatId, table.accountId] }),
    index("telegram_chat_ownerships_account_idx").on(table.accountId),
  ],
);
...
export const matchSettings = pgTable("match_settings", {
  ...
  positionMaxAgeSeconds: integer("position_max_age_seconds")
    .default(15)
    .notNull(),
  maxAccuracyMeters: doublePrecision("max_accuracy_meters")
    .default(50)
    .notNull(),
  maxSpeedMps: doublePrecision("max_speed_mps").default(15).notNull(),
  caughtBehavior: caughtBehaviorEnum("caught_behavior")
    .default("SPECTATOR")
    .notNull(),
  boundaryGraceSeconds: integer("boundary_grace_seconds").default(30).notNull(),
  boundaryAudience: boundaryAudienceEnum("boundary_audience")
    .default("HOST")
    .notNull(),
...
export const visibilityRules = pgTable(
  "visibility_rules",
  {
    ...
    visibleDurationSeconds: integer("visible_duration_seconds")
      .default(10)
      .notNull(),
  ...
  (table) => [
    uniqueIndex("visibility_rules_pair_uq").on(
      table.matchId,
      table.observerRole,
      table.targetRole,
    ),
...
export const participants = pgTable(
  ...
  (table) => [
    ...
    uniqueIndex("participants_match_account_uq").on(
      table.matchId,
      table.accountId,
    ),
  ],
...
export const invitations = pgTable(
  ...
  (table) => [
    ...
    index("invitations_match_idx").on(table.matchId),
  ],
);
...
export const locationSamples = pgTable(
  ...
  (table) => [
    ...
    index("location_samples_match_time_idx").on(
      table.matchId,
      table.recordedAt,
    ),
  ],
...
export const gameEvents = pgTable(
  "game_events",
  {
    ...
    targetParticipantId: uuid("target_participant_id").references(
      () => participants.id,
      { onDelete: "set null" },
    ),
  ...
  (table) => [
    index("game_events_match_time_idx").on(table.matchId, table.occurredAt),
  ],
);
