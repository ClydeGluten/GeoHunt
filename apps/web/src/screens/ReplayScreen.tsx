...
export function ReplayScreen({
  ...
  useEffect(() => {
    void Promise.all([
      api<ReplayData>(`/v1/matches/${matchId}/replay`),
      api<MatchSnapshot>(`/v1/matches/${matchId}`),
    ])
      .then(([data, match]) => {
        setReplay(data);
        setSnapshot(match);
  ...
  const times = useMemo(
    () =>
      [
        ...new Set(
          replay?.frames.map((frame) => new Date(frame.recordedAt).getTime()) ??
            [],
        ),
      ].sort((a, b) => a - b),
    [replay],
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(
      ...
      Math.min(
        1000,
        Math.max(80, (times[cursor + 1] ?? 0) - (times[cursor] ?? 0)),
      ),
    );
      ...
      const frame = replay.frames
        .filter(
          (item) =>
            ...
            new Date(item.recordedAt).getTime() <= currentTime,
        )
        .at(-1);
      if (!frame) return [];
      return [
        {
          ...
          role: participant.role as PlayerRole,
          stale: false,
          frozen: false,
        },
      ];
  ...
  const togglePublish = async () => {
    try {
      const result = await put<{ published: boolean }>(
        `/v1/matches/${matchId}/replay/publication`,
        { published: !published },
      );
  ...
  return (
    <main className="replay-shell">
      <div className="replay-map">
        <MapView polygon={snapshot.playzone} positions={positions} />
      </div>
      ...
      <section className="replay-controls">
        <div className="replay-meta">
          <span>
            {times.length
              ? new Date(times[cursor] ?? times[0] ?? 0).toLocaleTimeString(
                  [],
          ...
          <span>
            {replay.frames.length} points · {replay.events.length} events
          </span>
        </div>
        <input
        ...
        {replay.truncated && (
          <p className="warning-note">
            ...
            history is not included in this response.
          </p>
        )}
        {snapshot.viewerIsHost && (
          <p className="fine-print">
            {published
              ? "Participants can view this replay."
              : "Only you can view this replay until it is published."}
          </p>
        )}
      </section>
    </main>
