...
if (!file)
  throw new Error(
    "LOAD_CLIENTS_FILE must point to a JSON array of prepared player sessions",
  );
const clients = JSON.parse(await readFile(file, "utf8")) as LoadClient[];
if (clients.length !== 100)
  throw new Error(
    `Expected exactly 100 load clients, received ${clients.length}`,
  );
...
setTimeout(
  ...
  Math.max(5000, durationSeconds * 500),
);
