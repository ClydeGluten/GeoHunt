...
function runtime(emergencyReveal = false): MatchRuntime {
  return {
    match: {
      ...
      winnerRole: null,
      phaseStartedAt: new Date("2026-08-21T12:00:00Z"),
      phaseEndsAt: new Date("2026-08-21T13:00:00Z"),
      activeStartedAt: new Date("2026-08-21T12:00:00Z"),
      pausedAt: null,
      pausedDurationMs: 0,
      emergencyReveal,
    },
    ...
    locations: [
      {
        ...
        speedMps: null,
        headingDegrees: null,
        recordedAt: "2026-08-21T12:00:25.000Z",
        clientSequence: 1,
      },
      {
...
describe("coordinate authorization", () => {
  ...
  it("never serializes a target hidden from this viewer", async () => {
    ...
    expect(snapshot.winnerRole).toBeNull();
    expect(
      snapshot.visiblePositions.map((position) => position.participantId),
    ).toEqual(["hider"]);
    expect(JSON.stringify(snapshot)).not.toContain(
      'Secret seeker","role":"SEEKER","latitude',
    );
    expect(JSON.stringify(snapshot)).not.toContain("private-account-id");
    expect(snapshot.participants[0]).not.toHaveProperty("accountId");
  });
  ...
  it("notifies a player about emergency mode without granting host-only coordinates", async () => {
    ...
    vi.setSystemTime(new Date("2026-08-21T12:00:30Z"));
    const store = {
      getRuntime: vi.fn().mockResolvedValue(runtime(true)),
    } as unknown as GameStore;
    const snapshot = await buildSnapshot(store, redis, "match", viewer);
  ...
  it("gives a spectator host the referee visibility rules", async () => {
    ...
    const store = {
      getRuntime: vi.fn().mockResolvedValue(match),
    } as unknown as GameStore;
    const referee: ViewerContext = {
      participantId: "referee",
      accountId: "host",
      displayName: "Referee",
      role: "SPECTATOR",
      isHost: true,
  ...
  it("uses the host's chosen player role for visibility", async () => {
    ...
    const playingHost: ViewerContext = {
      ...viewer,
      accountId: "host",
      isHost: true,
    };
