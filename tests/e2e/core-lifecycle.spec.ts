...
async function authenticateHost(page: Page, baseURL: string) {
  page.on("pageerror", (error) =>
    console.error(`[host page error] ${error.message}`),
  );
  page.on("console", (message) => {
    if (message.type() === "error")
      console.error(`[host console] ${message.text()}`);
  });
  ...
  if (token) {
    ...
    await page.reload();
    await expect(
      page.getByRole("button", { name: "Create a hunt" }),
    ).toBeVisible({ timeout: 15_000 });
    return;
  }
  await page.getByLabel("Your trail name").fill(hostName);
  await page.getByRole("button", { name: "Continue in browser" }).click();
  await expect(page.getByRole("button", { name: "Create a hunt" })).toBeVisible(
    { timeout: 15_000 },
  );
}

test("core lifecycle survives missing pre-game geolocation", async ({
  browser,
  baseURL,
}) => {
  ...
  try {
    ...
    await guestPage
      .getByRole("button", { name: "Accept & join lobby" })
      .click();
    await expect(
      guestPage.getByText("SPECTATOR", { exact: true }),
    ).toBeVisible();
