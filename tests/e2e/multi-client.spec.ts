...
test("host, seeker, hider, match finish, and published replay", async ({
  browser,
  baseURL,
}) => {
  const host = await browser.newContext({
    geolocation: { latitude: 51.128, longitude: 71.43 },
    permissions: ["geolocation"],
  });
  const hostPage = await host.newPage();
  await hostPage.goto(baseURL!);
  await hostPage.getByLabel("Your trail name").fill("Multi-client Host");
  await hostPage.getByRole("button", { name: "Continue in browser" }).click();
  ...
  const guests = await Promise.all(
    ["Swift Fox", "Night Owl"].map(async (name, index) => {
      const context = await browser.newContext({
        geolocation: { latitude: 51.128 + index * 0.00002, longitude: 71.43 },
        permissions: ["geolocation"],
      });
  ...
  await guests[0]!.page.goto(
    `${baseURL}/?replay=${new URL(hostPage.url()).searchParams.get("replay")}`,
  );
